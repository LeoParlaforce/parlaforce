...
<body style="background:#000;color:#fff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:0;padding:48px 24px;">
  <div style="max-width:540px;margin:0 auto;">
    ...
    <h1 style="font-size:40px;font-weight:900;font-style:italic;text-transform:uppercase;letter-spacing:-0.03em;color:#fff;margin:0 0 20px;line-height:1;">Tu es dedans.</h1>
    <p style="color:#71717a;font-size:16px;line-height:1.75;margin:0 0 12px;font-style:italic;">
      Nouveaux articles, nouvelles dans la boutique, protocoles — tu seras parmi les premiers à le savoir.
    </p>
    <p style="color:#71717a;font-size:16px;line-height:1.75;margin:0 0 32px;font-style:italic;">
      Pas de spam. Que du concret.
    </p>
  ...
  try {
    ...
    await Promise.all([
      resend.emails.send({
        from: FROM_EMAIL,
        to: email,
        subject: 'Tu es dedans. — ParlaForce',
        html: welcomeHtml,
      }),
      resend.emails.send({
        from: FROM_EMAIL,
        ...
        html: `<p style="font-family:sans-serif;">Nouvel abonné : <strong>${email}</strong></p>`,
      }),
    ])
    ...
    return NextResponse.json({ error: 'Inscription échouée. Réessaie.' }, { status: 500 })
  }
}
